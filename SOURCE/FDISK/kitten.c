...
char catcontents[1024];	/* *** Maximum size of PROGRAM.%LANG% file *** */
...
/* char hit_end = 0; */   /* boolean flag, has file hit the end */
...
/* success variable removed, it never changed in the function.
*/
...
int get_line (int file, char *orig_str, int size)
{
  ...
  while (size > 0) {
    ...
    if (ch < 0) {
      ...
      /* test if the pointers are equal, if they aren't there should be content */
      if(str == orig_str)
