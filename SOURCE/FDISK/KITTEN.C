...
char *
processEscChars( char *line ); /* Converts c escape sequences to chars */
...
#pragma aux dos_open = "mov ax, 0x3d00"                                      \
                       "int 0x21"                                            \
                       "jnc noerror"                                         \
                       "mov ax, 0xffff"                                      \
                       "noerror:" parm[dx][ax] value[ax];

#pragma aux dos_read = "mov ah, 0x3f"                                        \
                       "int 0x21"                                            \
                       "jnc noerror"                                         \
                       "xor ax, ax"                                          \
                       "noerror:" parm[bx][dx][cx] value[ax];

#pragma aux dos_close = "mov ah, 0x3e"                                       \
                        "int 0x21" parm[bx];
...
int dos_open( char *filename, int mode )
{
   ...
   if ( mode ) {
      ; /* mode ignored - readonly supported */
   }
